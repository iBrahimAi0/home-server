...
interface DeleteBotConfirmModalProps {
  botId: string;
  botName: string;
  isOpen: boolean;
  onClose: () => void;
...
export function DeleteBotConfirmModal({ botId, botName, isOpen, onClose, onDeleted }: DeleteBotConfirmModalProps) {
      ...
      <div className="flex flex-col w-full max-w-md bg-[#0E121A] border border-[#1E273A] rounded-lg shadow-2xl overflow-hidden font-sans">
        ...
        <div className="p-4 space-y-3.5">
          ...
          <div className="flex items-start gap-3 p-3 rounded-md bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs">
            ...
            <div>
              <p className="font-semibold text-white">Remove &quot;{botName}&quot; from the dashboard?</p>
              <p className="text-amber-200/90 mt-1">
                The bot process will be stopped first if it&apos;s running, and its entry removed from configuration.
              </p>
              <p className="text-rose-300 mt-1.5 font-medium">
                The bot&apos;s files on disk are NOT deleted — only the dashboard configuration entry is removed.
              </p>
            </div>
          </div>
